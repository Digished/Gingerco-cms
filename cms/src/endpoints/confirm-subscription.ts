...
export const confirmSubscriptionEndpoint: PayloadHandler = async (req) => {
  ...
  if (!token) {
    return htmlResponse('Invalid Link', 'This confirmation link is invalid or has expired.', false)
  }
  ...
  try {
    ...
    if (result.docs.length === 0) {
      return htmlResponse('Link Not Found', 'This confirmation link is invalid or has already been used.', false)
    }

    const subscriber = result.docs[0]

    if (subscriber.status === 'subscribed') {
      return htmlResponse('Already Confirmed', 'Your subscription is already active. Welcome to Ginger & Co!', true)
    }
    ...
    if (resendKey) {
      ...
      await resend.emails.send({
        ...
        html: welcomeEmail({ firstName: subscriber.firstName as string, unsubscribeUrl, siteUrl }),
      })
    }
    ...
    req.payload.logger.error({ err }, '[confirm-subscription]')
